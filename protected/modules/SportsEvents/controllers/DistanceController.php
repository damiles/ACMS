...
class DistanceController extends Controller
{
	...
	public function actionCreate()
	{
		$model=new Distance;
			...
			foreach($langs as $i=>$item){
				$cont=new DistanceContent;
				$cont->lang=$item->idLang;
				$cont->title="Sin titulo";
				$cont->idDistance=$model->idDistance;
				if(!$cont->save()){
					$model->delete();
					$this->redirect(array('index'));
				}
			}
			$this->redirect(array('update','id'=>$model->idDistance));
		}else{
			//$this->redirect(array('index'));
                    throw new CHttpException(500,'Error saving distance.');
		}
	...
	public function actionUpdate($id)
	{
		...
		if(isset($_POST['Distance']))
		{
			...
			if($model->save()){
                                $contents=$_POST['DistanceContent'];
                                foreach($contents as $item){
                                        $contenido=DistanceContent::model()->findbyPk(array("idDistance"=>$item["idDistance"], "lang"=>$item["lang"]));
                                        $contenido->attributes=$item;
                                        $contenido->save();
                                }
				//$this->redirect(array('view','id'=>$model->idDistance));
                                Yii::app()->user->setFlash('status','Distancia guardada.');
				$this->refresh();
                        }
		}
	...
	{
		$model=Distance::model()->findByPk((int)$id);
		if($model===null)
